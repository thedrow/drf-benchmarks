...
class TestSerializer(serializers.ModelSerializer, CythonOrderedDictMixin):
    class Meta:
        model = RegularFieldsModel
        fields = list(data.keys()) + ['method']


class TestNestedSerializer(serializers.ModelSerializer, CythonOrderedDictMixin):
    fk = TestSerializer()
...
@pytest.mark.benchmark(
    group="ModelSerializer serialization with cythonized ordered dictionary",
    min_rounds=100,
    max_time=60,
    warmup=True
)
